...
export default function AssetsPage() {
  ...
  const fetchData = React.useCallback(async () => {
    ...
    try {
      ...
      // Manager fetches team requests
      if (isManager) {
        const teamRes = await assetAPI.getTeamRequests();
        setTeamRequests(teamRes.data || []);
      }

      // HR fetches all pending requests
      if (isHR) {
        const pendingRes = await assetAPI.getPending();
  ...
  const myAssets = myRequests.filter(r => r.status === 'ACKNOWLEDGED' || r.status === 'ALLOCATED');
  const myPendingRequests = myRequests.filter(r => !['ACKNOWLEDGED', 'REJECTED'].includes(r.status));
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
          {[
            ...
            ...(isManager ? [{ label: 'Team Pending Approval', value: managerPendingApproval.length, icon: <AlertCircle />, color: '#ef4444' }] : []),
            ...(isHR ? [
        ...
        {(isManager || isHR) && (
          <div style={{ display: 'flex', gap: '8px', borderBottom: '1px solid #e5e7eb', paddingBottom: '0' }}>
            {[
              ...
              ...(isHR ? [{ id: 'pending', label: 'Pending Approval' }] : []),
            ].map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id as 'my' | 'team' | 'pending')}
                style={{
                  padding: '12px 20px',
                  border: 'none',
